...
### R1-003 — Snooze / Wartezeit verlängern
**User Story**  
...
**Akzeptanzkriterien (Given/When/Then)**
1. **Given** ein Item ist `Wartet` oder `Kauf erlaubt`, **when** ich Snooze mit einem Preset (z. B. +24h, +7 Tage, +30 Tage) auswähle, **then** wird `kauf_erlaubt_ab` entsprechend in die Zukunft verschoben.
2. **Given** ein Item ist bereits `Kauf erlaubt`, **when** ich snooze, **then** wechselt es zurück in `Wartet`.
3. **Given** ein Item ist final `Gekauft` oder `Nicht gekauft`, **when** ich snooze öffnen möchte, **then** ist Snooze nicht verfügbar.
...
### R1-004 — Liste bei 100+ Items: Suche, Filter, Sortierung
**User Story**  
