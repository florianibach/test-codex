...
5. **Given** ich öffne die Liste ohne aktive Filterauswahl, **when** die Standardansicht geladen wird, **then** sind standardmäßig nur die Status `Wartet` und `Kauf erlaubt` ausgewählt.
6. **Given** ich nutze die Sortierung „Next ready“ ohne Status-Filter (alle Status sichtbar), **when** die Liste gerendert wird, **then** erscheinen zuerst `Kauf erlaubt` und danach `Wartet`, jeweils mit dem **frühesten** `kauf_erlaubt_ab` zuerst.
7. **Given** die Einträge mit `Kauf erlaubt` und `Wartet` wurden einsortiert, **when** weitere Status (`Gekauft`, `Nicht gekauft`) angezeigt werden, **then** folgen diese danach in umgekehrter Chronologie (neueste Entscheidungen zuerst).
8. **Given** ich kombiniere Suche + Filter + Sortierung, **when** ich die Ansicht aktualisiere, **then** bleibt das Ergebnis konsistent.
