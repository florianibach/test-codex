...
### (done) R1-004.1 — Default-Statusauswahl und „Next ready“-Reihenfolge
**User Story**  
Als Nutzer:in möchte ich in der Standardansicht sofort die relevanten, noch offenen Entscheidungen sehen, damit ich zuerst handlungsfähige oder bald handlungsfähige Items priorisieren kann.

**Akzeptanzkriterien (Given/When/Then)**
1. **Given** ich öffne die Liste ohne aktive Filterauswahl, **when** die Standardansicht geladen wird, **then** sind standardmäßig nur die Status `Wartet` und `Kauf erlaubt` ausgewählt.
2. **Given** ich nutze die Sortierung „Next ready“ ohne Status-Filter (alle Status sichtbar), **when** die Liste gerendert wird, **then** erscheinen zuerst `Kauf erlaubt` und danach `Wartet`, jeweils mit dem **frühesten** `kauf_erlaubt_ab` zuerst.
3. **Given** die Einträge mit `Kauf erlaubt` und `Wartet` wurden einsortiert, **when** weitere Status (`Gekauft`, `Nicht gekauft`) angezeigt werden, **then** folgen diese danach in umgekehrter Chronologie (neueste Entscheidungen zuerst).

**Out of Scope**
- Änderungen an fachlichen Statusdefinitionen.

---

### (done) R1-005 — Erweiterte Insights mit Trendfokus
**User Story**  
...
### (done) R1-006 — Leichtgewichtiges Mehrbenutzer-Modell ohne Login
**User Story**  
...
6. **Given** ich öffne den Profilwechsel, **when** bereits Profile existieren, **then** werden diese als klickbare Auswahl angezeigt und ich kann alternativ einen neuen Namen eingeben.
7. **Given** ich lege ein neues Profil an oder benenne ein Profil um, **when** ich speichere/wechsle, **then** lande ich in den Profile-Settings und kann Name + Einstellungen dieses Profils direkt verwalten.
8. **Given** ein neues Profil wird angelegt, **when** der Name bestätigt wird, **then** wird das Profil sofort mit Standardwerten (u. a. Stundenlohn und Währung) persistiert – es gibt keine Profile ohne diese Pflichtwerte.
...
### (done) R1-006.1 — Profile löschen
**User Story**  
Als Nutzer:in möchte ich ein nicht mehr benötigtes Profil löschen können, damit die Profilauswahl übersichtlich bleibt und veraltete Daten entfernt werden.

**Akzeptanzkriterien (Given/When/Then)**
1. **Given** ich öffne die Profile-Settings eines bestehenden Profils, **when** ich „Profil löschen“ auswähle und den Löschdialog bestätige, **then** wird das Profil inklusive zugehöriger Daten dauerhaft entfernt.
2. **Given** ich öffne den Löschdialog, **when** ich abbreche, **then** bleibt das Profil unverändert bestehen.
3. **Given** das aktuell aktive Profil wird gelöscht, **when** der Vorgang abgeschlossen ist, **then** werde ich direkt zur Profilauswahl weitergeleitet und kein gelöschtes Profil bleibt aktiv.
4. **Given** es existiert nur noch ein Profil, **when** ich dieses löschen möchte, **then** wird der Vorgang blockiert oder es muss unmittelbar ein neues Profil angelegt/gewählt werden, damit die App weiterhin einen gültigen Profilkontext hat.
5. **Given** ein Profil wurde gelöscht, **when** ich die Profilliste öffne, **then** ist es dort nicht mehr auswählbar.

**Out of Scope**
- Soft-Delete/Papierkorb mit Wiederherstellung.
- Teilweises Löschen einzelner Profildaten ohne Profilentfernung.

---

### (done) R1-007 — Tags als Dropdown statt nur Freitext
**User Story**  
...
### (done) R1-008 — Währung im Profil hinterlegen und konsistent anzeigen
**User Story**  
Als Nutzer:in möchte ich meine bevorzugte Währung im Profil als Freitext hinterlegen können (Standard: `€`), damit alle Preisangaben in meiner gewählten Währung klar und konsistent dargestellt werden.

**Akzeptanzkriterien (Given/When/Then)**
1. **Given** ich öffne mein Profil, **when** ich das Feld „Währung“ sehe, **then** ist es als Freitext editierbar und standardmäßig auf `€` gesetzt.
2. **Given** ich speichere eine eigene Währung (z. B. `CHF`, `$`, `EUR`), **when** ich die App weiter nutze, **then** bleibt diese Einstellung pro Profil persistent gespeichert.
3. **Given** in der App werden Preis- oder Summenfelder angezeigt (Liste, Detail, Formulare, Insights), **when** Werte gerendert werden, **then** wird die im Profil hinterlegte Währung bei allen Währungsfeldern angezeigt.
4. **Given** ein Profil hat keine explizite Währung gesetzt (z. B. Altbestand), **when** Währungsfelder angezeigt werden, **then** wird automatisch `€` als Fallback verwendet.

**Out of Scope**
- Automatische Wechselkurs-Umrechnung zwischen Währungen.
- Lokalisierte Zahlen-/Währungsformatierung je Land (z. B. Tausendertrennzeichen).
